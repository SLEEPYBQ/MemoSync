...
export interface LocalHttpServerInfo {
  ...
  processName?: string
  sameProject?: boolean
  /** Server authority for process termination; false for managed study previews. */
  canKill?: boolean
  /** True when the app runs in a container (PREVIEW_VIA_PROXY): the browser
   * iframe lives on the HOST, so `address` (in-container localhost) is
   * unreachable — open the app's localhost-subdomain proxy when local, or
   * its same-origin path proxy when MemoSync is reached remotely. */
  preferProxy?: boolean
}
...
export type ClientCommand =
  ...
  | {
      ...
      effort?: string
      planMode?: boolean
      /** Server-issued id for the one first prompt staged behind the opening Memory Board. */
  ...
  | { type: "chat.interruptMemory"; chatId: string; memoryId: string; quote?: string }
  | { type: "chat.resumeInterrupted"; chatId: string; interruptId: string; correction: string; selectedIds: string[]; enforce?: boolean }
  ...
  | { type: "chat.respondMemoryPreview"; chatId: string; previewId: string; decision: MemoryPreviewDecision; memoryIds?: string[]; expectedUses?: ExpectedMemoryUse[]; operationId?: string }
  | { type: "chat.respondMemoryProposals"; chatId: string; proposalsId: string; decision: "reviewed" | "skipped" }
  | { type: "chat.respondMemoryCheckup"; chatId: string; checkupId: string; decision: "handled" | "skipped" }
  | { type: "chat.respondMemoryTransfer"; chatId: string; transferId: string; decision: "handled" | "skipped" }
  | { type: "chat.reopenMemoryPreparation"; chatId: string; from: "proposals" | "checkup" | "transfer"; stageId: string }
  | {
