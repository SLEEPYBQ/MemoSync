...
import { memo, useCallback, useEffect, useRef, useState, type FocusEvent, type FormEvent, type MouseEvent as ReactMouseEvent, type ReactNode, type SyntheticEvent } from "react"
...
import { canKillLocalHttpServer, localServerPreviewUrl, manualBrowserAddressUrl } from "../../lib/localServerPreview"
...
interface BrowserPanelProps {
  ...
  /** Trusted local document used by the mounted Guide; production omits it. */
  previewDocument?: string
}

export function browserPreviewFrameAttributes(address: string, previewDocument?: string) {
  return previewDocument && address
    ? { src: undefined, srcDoc: previewDocument }
    : { src: address, srcDoc: undefined }
}
...
function BrowserPanelImpl({ projectId, socket, onRunQuickAction, previewDocument }: BrowserPanelProps) {
  ...
  const previewNavigationKey = `${address}-${iframeVersion}`
  const [previewFeedback, setPreviewFeedback] = useState<{
    key: string
    status: "idle" | "loading" | "ready" | "warning"
  }>({ key: previewNavigationKey, status: address ? "loading" : "idle" })
  ...
  const previewInspectionTimeoutsRef = useRef<number[]>([])
  ...
  const killServer = useCallback((server: LocalHttpServerInfo) => {
    setLocalServers(removeCachedLocalHttpServer(projectId, server.port))
  ...
  useEffect(() => {
    return () => {
      ...
      previewInspectionTimeoutsRef.current.forEach((timeoutId) => window.clearTimeout(timeoutId))
      previewInspectionTimeoutsRef.current = []
    }
  }, [])
  ...
  function handleAddressSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    navigateBrowser(
      projectId,
      manualBrowserAddressUrl(addressDraft, window.location, localServers.some((server) => server.preferProxy)),
    )
  }
  ...
  function handlePreviewLoad(event: SyntheticEvent<HTMLIFrameElement>) {
    ...
    const loadedKey = previewNavigationKey
    setPreviewFeedback({ key: loadedKey, status: "ready" })
    previewInspectionTimeoutsRef.current.forEach((timeoutId) => window.clearTimeout(timeoutId))
    previewInspectionTimeoutsRef.current = []
    if (!isInspectablePreviewAddress(address, window.location.origin)) return

    const inspect = () => {
      try {
        const document = loadedIframe.contentDocument
        return document ? inspectPreviewDocument(document) : "uninspectable"
      } catch {
        return "uninspectable"
      }
    }

    const firstInspection = window.setTimeout(() => {
      if (inspect() !== "blank") return
      const confirmation = window.setTimeout(() => {
        if (inspect() !== "blank") return
        setPreviewFeedback((current) => (
          current.key === loadedKey ? { key: loadedKey, status: "warning" } : current
        ))
      }, 900)
      previewInspectionTimeoutsRef.current.push(confirmation)
    }, 700)
    previewInspectionTimeoutsRef.current.push(firstInspection)
  }

  function handlePreviewError() {
    setPreviewFeedback({
      key: previewNavigationKey,
      status: isInspectablePreviewAddress(address, window.location.origin) ? "warning" : "ready",
    })
  }

  const effectivePreviewStatus = effectivePreviewFeedbackStatus(
    previewFeedback,
    previewNavigationKey,
    Boolean(address),
  )

  return (
    <div data-browser-panel="true" className="h-full min-h-0 border-l border-border bg-background md:min-w-[370px]">
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex shrink-0 items-center gap-1 border-b border-border px-2 py-2">
          ...
          <form onSubmit={handleAddressSubmit} className="group relative min-w-0 flex-1">
            <Input
              ...
              readOnly={Boolean(previewDocument)}
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          {!address ? (
                ...
                {serverError ? (
                  ...
                  <div className="space-y-1.5">
                    {visibleServers.map((server) => {
                      const openUrl = localServerPreviewUrl(server)
                      return (
                      <ContextMenu key={server.address}>
                        <ContextMenuTrigger asChild>
                          <div
                            role="button"
                            tabIndex={0}
                            data-browser-server-card-current-project={server.sameProject ? "true" : "false"}
                            onClick={() => navigateBrowser(projectId, openUrl)}
                            onKeyDown={(event) => {
                              if (event.key !== "Enter" && event.key !== " ") return
                              event.preventDefault()
                              navigateBrowser(projectId, openUrl)
                            }}
                          ...
                          >
                            ...
                            <span className="flex w-full min-w-0 items-center gap-3">
                              <span className="min-w-0 flex-1 truncate text-xs text-muted-foreground">{openUrl}</span>
                              {server.ownerPath ? (
                        ...
                        {!previewDocument ? <ContextMenuContent>
                          <ContextMenuItem
                            onSelect={(event) => {
                              event.preventDefault()
            ...
            <div
              data-browser-preview-state={effectivePreviewStatus}
              className="relative h-full w-full overflow-auto bg-muted/20"
            >
              <iframe
                ref={iframeRef}
                key={`${address}-${iframeVersion}`}
                {...browserPreviewFrameAttributes(address, previewDocument)}
                title="Browser panel"
              ...
              />
              {effectivePreviewStatus === "loading" ? <BrowserPreviewLoading /> : null}
              {effectivePreviewStatus === "warning" ? (
                <BrowserPreviewRecovery
                  onRefresh={handleRefresh}
                  onHome={() => navigateBrowser(projectId, "")}
                />
              ) : null}
            </div>
          )}
