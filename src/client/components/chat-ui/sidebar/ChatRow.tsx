...
const loadingStatuses = new Set(["starting", "previewing_memory", "running"])

interface Props {
  chat: SidebarChatRow
  /** Study sessions hide forking because it inherits transcript/session state. */
  showFork?: boolean
...
function ChatRowImpl({
  ...
  const row = (
    ...
    >
      ...
      <div className={cn("relative h-7 mr-[2px] shrink-0", showFork && chat.canFork ? "w-12" : "w-6")}>
        {trailingLabel ? (
          showShortcutKeycap ? (
