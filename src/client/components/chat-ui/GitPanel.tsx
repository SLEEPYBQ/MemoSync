...
import { AlertTriangle, ArrowUp, Ban, Check, ChevronDown, ChevronUp, Columns2, Copy, Download, Ellipsis, FolderOpen, GitBranch, GitBranchPlus, GitMerge, GitPullRequest, LoaderCircle, Minus, PenLine, RefreshCw, Rows3, Search, Sparkles, Trash2, Upload, WrapText } from "lucide-react"
...
function BranchSwitcher({
  ...
  const [query, setQuery] = useState("")
  // Study deployments are local-only git: the PR view has nothing to show.
  const remoteGitEnabled = !useConditionPolicy().studyMode
...
function DiffFileCard({
  ...
  function handleToggleRequest() {
    ...
    // Expand whether the load succeeds OR fails: on failure the error lives in
    // patchError, which only renders in the expanded body. Keeping the card
    // collapsed on failure (the old .catch(() => {})) left the file looking
    // normal but permanently un-openable, with the error invisible (BUG PANEL-4).
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <div ref={cardRef} key={file.path} className="relative rounded-lg border border-border bg-background">
          ...
          {!isCollapsed ? (
            <div className="memosync-diff-patch overflow-hidden rounded-b-[calc(theme(borderRadius.lg)-1px)] pb-[1px]">
              {previewAttachment ? (
...
function GitPanelImpl({
  ...
  const fileActions: DiffFileActions = useMemo(() => ({
    ...
    onCopyFilePath,
    onCopyRelativePath,
  }), [onOpenInFinder, onDiscardFile, onIgnoreFile, onIgnoreFolder, onCopyFilePath, onCopyRelativePath])
  // Study deployments are local-only git: no publish/sync/PR affordances.
  const remoteGitEnabled = !useConditionPolicy().studyMode
  ...
  const canCommit = diffs.status === "ready"
    ...
    && hasSummary
    && !isBusy
  const primaryCommitMode: DiffCommitMode = hasRemoteOrigin && remoteGitEnabled ? "commit_and_push" : "commit_only"
    ...
    <div className="h-full min-h-0 border-l border-border bg-background md:min-w-[370px]">
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex shrink-0 items-center gap-2 border-b border-border pl-2.5 pr-2 h-[49px]">
          ...
          {diffs.status === "ready" ? (
            // Local-only git: repos without a remote — or a study deployment
            // with remote git turned off — get no publish/sync CTA.
            !hasRemoteOrigin || !remoteGitEnabled ? null : syncAction === "publish" ? (
              <Button
                variant="ghost"
                size="sm"
