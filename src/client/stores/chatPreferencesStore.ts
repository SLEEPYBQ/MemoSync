...
export const useChatPreferencesStore = create<ChatPreferencesState>()(
  ...
  {
    // Persist per-chat composer prefs (model / effort / context window / plan
    // mode) so they survive a refresh. The store already carried the full
    // migration machinery but was no longer wrapped in persist, so every
    // reload reset each chat's composer to the provider defaults (BUG INPUT-1).
    ...
    merge: (persisted, current) => ({
      ...current,
      ...migrateChatPreferencesState(persisted as Partial<PersistedChatPreferencesState> | undefined),
    }),
  },
  )
)
