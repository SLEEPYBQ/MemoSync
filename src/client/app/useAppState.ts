...
import { canCancelStatus, getLatestToolIds, hasPendingMemoryPreview, isProcessingStatus } from "./derived"
import { AppSocket, type SocketStatus } from "./socket"
import { useServerProviders } from "../lib/conditionApi"
import type { EditorOpenSettings, OpenExternalAction } from "../../shared/protocol"
import { DISPLAY_NAME, SELF_UPDATE_ENABLED } from "../../shared/branding"

const CHAT_SNAPSHOT_STALE_MS = 20_000

export function shouldRefreshStaleChatSnapshot(input: {
  ...
  return Boolean(input.activeChatId)
    ...
    && input.now - input.lastSnapshotAt >= CHAT_SNAPSHOT_STALE_MS
    && input.now - input.lastRefreshAt >= CHAT_SNAPSHOT_STALE_MS
}
...
function sameMemoryRecordEntries(
  left: ChatSnapshot["memoryRecordMessages"] | null | undefined,
  right: ChatSnapshot["memoryRecordMessages"] | null | undefined,
) {
  if (left === right) return true
...
function sameProviders(left: ProviderCatalogEntry[] | null | undefined, right: ProviderCatalogEntry[] | null | undefined) {
  ...
  return left.every((provider, index) => {
    ...
    return Boolean(other)
      ...
      && provider.defaultEffort === other.defaultEffort
      && provider.supportsPlanMode === other.supportsPlanMode
      && provider.efforts.length === other.efforts.length
      && provider.efforts.every((effort, effortIndex) => {
        const otherEffort = other.efforts[effortIndex]
        return Boolean(otherEffort) && effort.id === otherEffort.id && effort.label === otherEffort.label
      })
      ...
      && provider.models.every((model, modelIndex) => {
        ...
        return Boolean(otherModel)
          ...
          && model.supportsMaxReasoningEffort === otherModel.supportsMaxReasoningEffort
          && sameStringArray(model.aliases, otherModel.aliases)
          && (model.contextWindowOptions?.length ?? 0) === (otherModel.contextWindowOptions?.length ?? 0)
      })
  })
}
...
function sameChatSnapshotCore(left: ChatSnapshot | null, right: ChatSnapshot | null) {
  ...
  return sameRuntime(left.runtime, right.runtime)
    ...
    && sameMemoryRecordEntries(left.memoryRecordMessages, right.memoryRecordMessages)
...
function useAppSocket() {
  const socketRef = useRef<AppSocket | null>(null)
  if (!socketRef.current) {
    socketRef.current = new AppSocket(wsUrl())
  }
  ...
  return socketRef.current as AppSocket
}
...
function logSendToStartingTrace(
  ...
  console.debug("[memosync/send->starting][client]", {
  ...
  if (options?.provider === "claude" && options.model && options.modelOptions?.claude) {
    return {
      ...
      modelOptions: {
        ...
        contextWindow: normalizeClaudeContextWindow(options.model, options.modelOptions.claude.contextWindow),
      },
...
const UI_UPDATE_RESTART_STORAGE_KEY = "memosync:ui-update-restart"
const UI_UPDATE_RELOAD_REQUEST_STORAGE_KEY = "memosync:last-update-reload-request"
...
export async function runChatStartAction(
  action: () => Promise<void>,
...
export function resolveComposeIntent(params: {
  /** A project the user explicitly picked (landing composer) — beats any remembered selection. */
  explicitLocalPath?: string | null
  selectedProjectId: string | null
  sidebarProjectId?: string | null
  fallbackLocalProjectPath?: string | null
}): StartChatIntent | null {
  if (params.explicitLocalPath) {
    return { kind: "local_path", localPath: params.explicitLocalPath }
  }
...
export interface AppState {
  ...
  memoryRecordMessages: ReturnType<typeof processTranscriptMessages>
  ...
  handleCreateChat: (projectId: string) => Promise<ChatStartResult>
  ...
  handleSend: (content: string, options?: { provider?: AgentProvider; model?: string; modelOptions?: ModelOptions; planMode?: boolean; attachments?: import("../../shared/types").ChatAttachment[]; projectLocalPath?: string; openingReviewId?: string }) => Promise<void>
  ...
  handleMemoryPreviewRespond: (previewId: string, decision: MemoryPreviewDecision, memoryIds?: string[], expectedUses?: ExpectedMemoryUse[]) => Promise<void>
  handleMemoryProposalsRespond: (proposalsId: string, decision: "reviewed" | "skipped") => Promise<void>
  handleMemoryCheckupRespond: (checkupId: string, decision: "handled" | "skipped") => Promise<void>
  handleMemoryTransferRespond: (transferId: string, decision: "handled" | "skipped") => Promise<void>
  handleMemoryPreparationReopen: (from: "proposals" | "checkup" | "transfer", stageId: string) => Promise<void>
...
export function useAppState(activeChatId: string | null): AppState {
  ...
  // A command error belongs to the surface it happened on — it must not
  // follow the user to the landing page or the next chat.
  useEffect(() => {
  ...
  const lastChatSnapshotAtRef = useRef(0)
  const lastChatRefreshAtRef = useRef(0)
  const lastStartingRenderedTraceIdRef = useRef<string | null>(null)
  // In-flight guard so a double-click never creates two chats (BUG LAND-3).
  ...
  useEffect(() => {
    ...
    lastChatRefreshAtRef.current = 0
    logAppState("subscribing to chat", {
      subscriptionId,
      activeChatId,
    ...
    return () => {
      logAppState("unsubscribing from chat", {
        subscriptionId,
        activeChatId,
        sidebarProjectGroups: sidebarProjectGroups.length,
        sidebarChatCount: sidebarProjectGroups.reduce((count, group) => count + group.chats.length, 0),
  ...
  const memoryRecordMessages = useMemo(
    () => processTranscriptMessages(activeChatSnapshot?.memoryRecordMessages ?? serverTranscriptEntries),
    [activeChatSnapshot?.memoryRecordMessages, serverTranscriptEntries],
  )
  ...
  // Launcher fallback honors the server catalog (study pin included) — the
  // static client-side PROVIDERS list only bridges the very first fetch.
  const availableProviders = activeChatSnapshot?.availableProviders ?? serverProviders
  const isProcessing = isProcessingStatus(effectiveRuntimeStatus ?? undefined)

  // A live WebSocket can still lose one terminal chat snapshot under
  // backpressure. If that snapshot was the transition to idle, the UI would
  // otherwise display Running forever even though the server is done. While
  // this chat is visibly processing, periodically rebuild only its snapshot;
  // the server clears the subscription signature and returns current truth.
  useEffect(() => {
    if (!activeChatId || !isProcessing || connectionStatus !== "connected") return
    const refreshIfStale = () => {
      const now = Date.now()
      if (!shouldRefreshStaleChatSnapshot({
        activeChatId,
        isProcessing,
        connectionStatus,
        now,
        lastSnapshotAt: lastChatSnapshotAtRef.current,
        lastRefreshAt: lastChatRefreshAtRef.current,
      })) return
      lastChatRefreshAtRef.current = now
      socket.refreshTopic({ type: "chat", chatId: activeChatId, recentLimit: INITIAL_CHAT_RECENT_LIMIT })
    }
    const timer = window.setInterval(refreshIfStale, 5_000)
    return () => window.clearInterval(timer)
  }, [activeChatId, connectionStatus, isProcessing, socket])

  // A parked preview gate has no ActiveTurn (status reads idle) but the turn
  // IS busy server-side — sends must queue, not pretend to deliver.
  const pendingMemoryPreview = useMemo(() => hasPendingMemoryPreview(messages), [messages])
  ...
  const startChatFromIntent = useCallback(async (intent: StartChatIntent): Promise<ChatStartResult> => {
    // Guard every new-chat entry point against a double-click / concurrent
    // invocation during the async create window. The sidebar's "New Chat"
    // button had no disabled state wired for the project_id path, so two quick
    // clicks created two chats (BUG LAND-3). This source-level guard covers all
    // callers regardless of per-button UI plumbing.
    if (creatingChatRef.current) {
      return { ok: false, error: "Another chat is already being created. Please wait and try again." }
    }
    creatingChatRef.current = true
    try {
      return await runChatStartAction(async () => {
  ...
  const handleCreateChat = useCallback(async (projectId: string) => {
    return await startChatFromIntent({ kind: "project_id", projectId })
  }, [startChatFromIntent])
    ...
    try {
      ...
      if (!result.ok) {
        ...
        await dialog.alert({
          ...
          description: result.userMessage ?? `${DISPLAY_NAME} could not install the update. Try again later.`,
          closeLabel: "OK",
        })
        return
      }
  ...
  const handleSend = useCallback(async (
    content: string,
    options?: { provider?: AgentProvider; model?: string; modelOptions?: ModelOptions; planMode?: boolean; attachments?: import("../../shared/types").ChatAttachment[]; projectLocalPath?: string; openingReviewId?: string }
  ) => {
    const attachments = options?.attachments ?? []
    if (activeChatId && (isProcessing || pendingMemoryPreview)) {
      try {
    ...
    try {
      ...
      if (!activeChatId) {
        const intent = resolveComposeIntent({
          explicitLocalPath: options?.projectLocalPath ?? null,
          selectedProjectId,
          sidebarProjectId: sidebarProjectGroups[0]?.groupKey ?? null,
          fallbackLocalProjectPath,
        })
        if (!intent) {
        ...
        if (intent.kind === "local_path") {
          ...
          setSelectedProjectId(projectId)
        } else if (intent.kind === "project_id") {
      ...
      const result = await socket.command<{ chatId?: string }>({
        ...
        modelOptions: options?.modelOptions,
        planMode: options?.planMode,
        openingReviewId: options?.openingReviewId,
      })
  ...
  }, [activeChatId, fallbackLocalProjectPath, isProcessing, navigate, optimisticUserPrompts, pendingMemoryPreview, selectedProjectId, serverTranscriptEntries, sidebarProjectGroups, socket])
  ...
  const handleOpenLocalLink = useCallback(async (
    ...
    action: OpenExternalAction = "open_default",
    editor?: EditorOpenSettings,
  ) => {
    try {
      await openExternal({
  ...
  const handleShareChat = useCallback(async (chatId: string | null | undefined = activeChatId) => {
    ...
    const confirmed = await dialog.confirm({
      ...
      description: "This writes a standalone web page containing the full chat history, tool calls, and all attachments into the project's exports folder. Nothing is uploaded.",
      confirmLabel: "Export",
      cancelLabel: "Cancel",
    })
    if (!confirmed) {
    ...
    if (result?.ok) {
      setStandaloneShareUrl(result.outputDir)
  ...
  const handleOpenStandaloneShareLink = useCallback(() => {
    ...
    void openExternal({ action: "open_finder", localPath: standaloneShareUrl }).catch((error) => {
      setCommandError(error instanceof Error ? error.message : String(error))
    })
    setStandaloneShareUrl(null)
  }, [openExternal, standaloneShareUrl])
  ...
  const handleMemoryPreviewRespond = useCallback(async (previewId: string, decision: MemoryPreviewDecision, memoryIds?: string[], expectedUses?: ExpectedMemoryUse[]) => {
    if (!activeChatId) return
    try {
      await socket.command({
        type: "chat.respondMemoryPreview",
        chatId: activeChatId,
        previewId,
        decision,
        memoryIds,
        expectedUses,
        operationId: `control:working-memory-${decision}:${previewId}:${crypto.randomUUID()}`,
      })
    } catch (error) {
      setCommandError(error instanceof Error ? error.message : String(error))
      // Re-throw so the gate card can re-enable its buttons instead of being
      // stuck on a spinner forever when the send fails (BUG MSG-5).
      throw error
    }
  }, [activeChatId, socket])

  const handleMemoryProposalsRespond = useCallback(async (proposalsId: string, decision: "reviewed" | "skipped") => {
    if (!activeChatId) return
    try {
      await socket.command({
        type: "chat.respondMemoryProposals",
        chatId: activeChatId,
        proposalsId,
        decision,
      })
  ...
  const handleMemoryTransferRespond = useCallback(async (transferId: string, decision: "handled" | "skipped") => {
    if (!activeChatId) return
    try {
      await socket.command({
        type: "chat.respondMemoryTransfer",
        chatId: activeChatId,
        transferId,
        decision,
      })
  ...
  const handleMemoryCheckupRespond = useCallback(async (checkupId: string, decision: "handled" | "skipped") => {
    if (!activeChatId) return
    try {
      await socket.command({
        type: "chat.respondMemoryCheckup",
        chatId: activeChatId,
        checkupId,
        decision,
      })
  ...
  const handleMemoryPreparationReopen = useCallback(async (from: "proposals" | "checkup" | "transfer", stageId: string) => {
    if (!activeChatId) return
    try {
      await socket.command({
        type: "chat.reopenMemoryPreparation",
        chatId: activeChatId,
        from,
        stageId,
      })
  ...
  return {
    ...
    handleAskUserQuestion,
    handleExitPlanMode,
    handleMemoryPreviewRespond,
    handleMemoryProposalsRespond,
    handleMemoryCheckupRespond,
    handleMemoryTransferRespond,
    handleMemoryPreparationReopen,
