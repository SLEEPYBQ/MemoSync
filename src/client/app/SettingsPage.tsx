...
const sidebarItems = [
  ...
  },
  {
    ...
    subtitle: "Manage the default chat provider and saved DeepSeek model defaults.",
  },
  {
    id: "keybindings",
    label: "Keybindings",
  ...
  ...(SELF_UPDATE_ENABLED ? [changelogSidebarItem] : []),
] as const
...
// Delegating mode (F4's two control styles): Ask parks every proposal for
// review; Auto applies non-sensitive ones immediately with an auto badge +
// one-click revert.
const evolutionPolicyOptions = [
  { value: "ask" as const, label: "Ask" },
  { value: "auto" as const, label: "Auto" },
]

// Point this at your fork's releases endpoint before enabling SELF_UPDATE_ENABLED.
const GITHUB_RELEASES_URL = "https://api.github.com/repos/OWNER/MemoSync/releases"
...
const KEYBINDING_ACTIONS = (Object.keys(KEYBINDING_ACTION_LABELS) as KeybindingAction[])
  .filter((action) => action !== "openInEditor")
...
export function ChangelogSection({
  ...
  return (
    <div className="space-y-4">
      ...
      {SELF_UPDATE_ENABLED && !canInstallUpdate && status === "success" ? (
            ...
            <div className="flex flex-row items-center min-w-0 flex-1 gap-3 ">
              ...
              <div className="flex flex-row items-center justify-end min-w-0 flex-1 gap-2 ">
                ...
                >
                  ...
                  {SELF_UPDATE_ENABLED && isLatestRelease && canInstallUpdate ? (
                  <SettingsHeaderButton
                    variant="default"
                    className=""
...
export function SkillsSection({
  ...
  state: Pick<AppState, "connectionStatus" | "socket">
}) {
  const socket = state.socket
  const connectionStatus = state.connectionStatus
  // Study deployments cut outbound registry access; the installed list stays
  // (local lock-file reads only). The server refuses search/install too.
  const remoteSkillsEnabled = !useConditionPolicy().studyMode
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {remoteSkillsEnabled ? <section className="flex flex-col gap-3">
...
export function SettingsPage() {
  ...
  // Memory evolution delegating mode — undefined = still loading (the control
  // stays read-only until the truth lands, same as the panel it moved from).
  const [evolutionMode, setEvolutionMode] = useState<"ask" | "auto" | undefined>(undefined)
  ...
  useEffect(() => {
    if (selectedPage !== "general" || isConnecting) return
    void memoriesApi
      .getEvolutionPolicy()
      .then((r) => setEvolutionMode(r.mode))
      .catch(() => setEvolutionMode("ask"))
  }, [isConnecting, selectedPage])

  useEffect(() => {
    if (!SELF_UPDATE_ENABLED || selectedPage !== "changelog" || isConnecting) return
  ...
  async function handleMemoryPreviewChange(patch: { enabled?: boolean; autoProceedWhenEmpty?: boolean }) {
    try {
      setAppSettingsError(null)
      await handleWriteAppSettings({ memoryPreview: patch })
    } catch (error) {
      setAppSettingsError(error instanceof Error ? error.message : "Unable to save memory preview settings.")
    }
  }

  async function handleEvolutionModeChange(mode: "ask" | "auto") {
    if (evolutionMode === undefined || mode === evolutionMode) return
    const previous = evolutionMode
    setEvolutionMode(mode)
    try {
      await memoriesApi.setEvolutionPolicy(mode)
    } catch (error) {
      setEvolutionMode(previous)
      setAppSettingsError(error instanceof Error ? error.message : "Unable to save memory evolution settings.")
    }
  }
              ...
              <div className="mx-auto max-w-4xl">
                <div className="pb-6">
                  <div className="flex items-center justify-between gap-4 min-h-[34px]">
                    ...
                    {selectedPage === "general" && SELF_UPDATE_ENABLED ? (
                      <SettingsHeaderButton
                        variant="outline"
                ...
                {selectedPage === "general" ? (
                  <>
                    ...
                    <div className="border-b border-border">
                      ...
                      {conditionPolicy.preview ? (
                        <>
                          <SettingsRow
                            title="Memory Preview"
                            description="Before each turn, show which memories the agent brings and how they shape its plan — you decide to go on, proceed without memory, or dismiss."
                          >
                            <SegmentedControl
                              value={appSettings?.memoryPreview?.enabled === false ? "disabled" : "enabled"}
                              onValueChange={(value) => {
                                void handleMemoryPreviewChange({ enabled: value === "enabled" })
                              }}
                          ...
                          <SettingsRow
                            title="Auto-proceed When No Memory Applies"
                            description="Skip the preview pause when the plan involves no memories (the preview is still recorded in the transcript)."
                          >
                            <SegmentedControl
                              value={appSettings?.memoryPreview?.autoProceedWhenEmpty === false ? "disabled" : "enabled"}
                              onValueChange={(value) => {
                                void handleMemoryPreviewChange({ autoProceedWhenEmpty: value === "enabled" })
                              }}
                              options={enabledDisabledOptions}
                              size="sm"
                            />
                      ...
                      <SettingsRow
                        title="Memory Evolution: Ask / Auto"
                        description="Ask parks every proposal for your review. Auto auto-applies non-sensitive proposals immediately — each still shows in the transcript with a one-click Revert (sensitive proposals still ask)."
                      >
                        <SegmentedControl
                          value={evolutionMode === "auto" ? "auto" : "ask"}
                          onValueChange={(value) => void handleEvolutionModeChange(value)}
                          options={evolutionPolicyOptions}
                          size="sm"
                        />
                      </SettingsRow>
