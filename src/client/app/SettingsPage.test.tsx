...
describe("fetchGithubReleases", () => {
  test("filters draft releases and sends the GitHub accept header", async () => {
    ...
    expect(requestedUrl).toBe("https://api.github.com/repos/OWNER/MemoSync/releases")
...
describe("resolveSettingsSectionId", () => {
  test("accepts known settings sections", () => {
    ...
    expect(resolveSettingsSectionId("changelog")).toBeNull()
...
describe("ChangelogSection", () => {
  ...
  test("hides update controls while the MemoSync release channel is disabled", () => {
    const html = renderToStaticMarkup(
      <ChangelogSection
        status="success"
        releases={SAMPLE_RELEASES}
  ...
  test("does not trust a stale available-update snapshot", () => {
    const html = renderToStaticMarkup(
      <ChangelogSection
        status="success"
        releases={SAMPLE_RELEASES}
