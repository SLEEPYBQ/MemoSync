...
const originalRuntimeProfile = process.env.MEMOSYNC_RUNTIME_PROFILE
...
afterEach(async () => {
  if (originalRuntimeProfile === undefined) {
    ...
    process.env.MEMOSYNC_RUNTIME_PROFILE = originalRuntimeProfile
  }
...
describe("EventStore", () => {
  test("uses the runtime profile for the default data dir", () => {
    ...
    expect(store.dataDir).toEndWith("/.memosync-dev/data")
  })
  ...
  test("a failed write does not poison the write chain — later writes still persist (WS-1)", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    await store.appendMessage(chat.id, entry("user_prompt", 1, { content: "ok" }))

    // Sabotage the next write: replace the transcripts dir with a FILE so the
    // append's mkdir throws. Before the fix, `writeChain = writeChain.then(task)`
    // with no catch left the chain permanently rejected, so EVERY later write's
    // callback silently never ran.
    const transcriptsDir = join(dataDir, "transcripts")
    await rm(transcriptsDir, { recursive: true, force: true })
    await writeFile(transcriptsDir, "block")
    await expect(store.appendMessage(chat.id, entry("assistant_text", 2, { text: "fails" }))).rejects.toThrow()

    await rm(transcriptsDir, { force: true })
    // The chain must have recovered: a subsequent write to a DIFFERENT log file
    // (the chats log) still applies. Before the fix the chain stayed rejected
    // and this callback (and every later one) never ran — the await would hang.
    await store.renameChat(chat.id, "Recovered")
    expect(store.getChat(chat.id)?.title).toBe("Recovered")
  })
  ...
  test("conditional transcript append revalidates inside the serialized write task", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    const queued = store.appendMessage(
      chat.id,
      entry("assistant_text", 202, { text: "stale" }),
      { shouldAppend: () => stillFresh },
    )
    // The predicate belongs to the queued task, not the call site. Mutating
    // before that task executes must suppress both durable and in-memory state.
    stillFresh = false

    expect(await queued).toBe(false)
    expect(store.getMessages(chat.id)).toHaveLength(0)
    expect(existsSync(join(dataDir, "transcripts", `${chat.id}.jsonl`))).toBe(false)

    stillFresh = true
    expect(await store.appendMessage(
      chat.id,
      entry("assistant_text", 203, { text: "fresh" }),
      { shouldAppend: () => stillFresh },
    )).toBe(true)
    expect(store.getMessages(chat.id)).toEqual([
      entry("assistant_text", 203, { text: "fresh" }),
  ...
  test("scrubs candidate content from legacy transcript rows during startup", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    await store.appendMessage(chat.id, {
      _id: "candidate-legacy",
      createdAt: 200,
      kind: "memory_candidates",
      candidates: [{
        id: "M-01",
        content: "legacy-local-secret",
        detail: "legacy-person@example.com",
        type: "fact",
        scope: "personal",
    ...
    const transcript = await readFile(join(dataDir, "transcripts", `${chat.id}.jsonl`), "utf8")
    expect(transcript).not.toContain("legacy-local-secret")
    expect(transcript).not.toContain("legacy-person@example.com")
    expect(reloaded.getMessages(chat.id)[0]).toMatchObject({
      kind: "memory_candidates",
      candidates: [{ id: "M-01" }],
    })
  })

  test("recovers compacted projects and chats when the primary snapshot is corrupt", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    await writeFile(join(dataDir, "snapshot.json"), "{broken", "utf8")

    const recovered = new EventStore(dataDir)
    await recovered.initialize()

    expect(recovered.getProject(project.id)?.localPath).toBe("/tmp/project")
    expect(recovered.getChat(chat.id)?.projectId).toBe(project.id)
  })

  test("falls back to the backup when the primary snapshot is parseable but structurally invalid", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    await writeFile(join(dataDir, "snapshot.json"), JSON.stringify({
      v: 2,
      generatedAt: 0,
      projects: [{}],
      chats: [],
    ...
    await recovered.initialize()
    expect(recovered.getProject(project.id)?.localPath).toBe("/tmp/project")
    expect(recovered.listProjects()).toHaveLength(1)
  })

  test("falls back to the backup when a primary snapshot contains malformed queued messages", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    await recovered.initialize()
    expect(recovered.getProject(project.id)?.localPath).toBe("/tmp/project")
    expect(recovered.getQueuedMessages(chat.id)).toEqual([])
  })

  test("keeps compacted state when a log contains a corrupt middle line", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    await writeFile(join(dataDir, "projects.jsonl"), "{broken\n{}\n", "utf8")
    const recovered = new EventStore(dataDir)
    await recovered.initialize()
    expect(recovered.getProject(project.id)?.localPath).toBe("/tmp/project")
    expect(await readFile(join(dataDir, "snapshot.json.bak"), "utf8")).toContain(project.id)
  ...
  test("skips a parseable event with missing required fields instead of failing startup", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    ...
    await recovered.initialize()
    expect(recovered.getProject(project.id)?.localPath).toBe("/tmp/project")
    expect(recovered.getProject("broken")).toBeNull()
  })

  test("does not let a parseable malformed event overwrite valid state with undefined fields", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    const project = await store.openProject("/tmp/project")
    const chat = await store.createChat(project.id)
    await store.renameChat(chat.id, "Good title")
    await store.compact()
    ...
    const recovered = new EventStore(dataDir)
    await recovered.initialize()
    expect(recovered.getChat(chat.id)?.title).toBe("Good title")
  })

  test("serializes compaction behind queued writes so it cannot truncate a just-appended event", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()

    const opening = store.openProject(join(dataDir, "race-project"), "Race Project")
    await store.compact()
    const project = await opening

    const reloaded = new EventStore(dataDir)
    await reloaded.initialize()
    expect(reloaded.listProjects().map((item) => item.id)).toContain(project.id)
  })
  ...
  test("hydrates the durable cancellation marker for a queued message after restart", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    await store.appendMessage(chat.id, {
      _id: "cancelled-queue-marker",
      createdAt: Date.now(),
      kind: "interrupted",
      cancelledQueuedMessageId: queued.id,
    })

    const reloaded = new EventStore(dataDir)
    await reloaded.initialize()
    expect(reloaded.getQueuedMessages(chat.id).map((message) => message.id)).toEqual([queued.id])
    expect(reloaded.getMessages(chat.id)).toContainEqual(expect.objectContaining({
      kind: "interrupted",
      cancelledQueuedMessageId: queued.id,
    }))
  })

  test("preserves queued messages without optional provider settings across compaction", async () => {
    const dataDir = await createTempDataDir()
    const store = new EventStore(dataDir)
    await store.initialize()
    ...
    const reloaded = new EventStore(dataDir)
    await reloaded.initialize()
    expect(reloaded.getQueuedMessages(chat.id)).toEqual([queued])
  })
