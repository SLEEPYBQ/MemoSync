...
interface AppSettingsFile {
  ...
  memoryPreview?: {
    enabled?: unknown
    autoProceedWhenEmpty?: unknown
  }
}

function normalizeMemoryPreview(value: AppSettingsFile["memoryPreview"] | undefined): MemoryPreviewSettings {
  return {
    enabled: typeof value?.enabled === "boolean" ? value.enabled : true,
    autoProceedWhenEmpty: typeof value?.autoProceedWhenEmpty === "boolean" ? value.autoProceedWhenEmpty : true,
  }
}
...
function normalizeAppSettings(
  ...
  const state: AppSettingsState = {
    ...
    memoryPreview: normalizeMemoryPreview(source?.memoryPreview),
    warning: null,
...
function toComparablePayload(source: AppSettingsFile) {
  return {
    ...
    memoryPreview: source.memoryPreview,
  }
}
...
export class AppSettingsManager {
  ...
  private startWatching() {
    ...
    try {
      ...
      // A watched directory can vanish under the watcher (temp data dirs in
      // tests, aggressive cleanup tools). Without a listener that surfaces as
      // an UNCAUGHT 'error' event and kills the process.
      this.watcher.on("error", (error: unknown) => {
        console.warn(`${LOG_PREFIX} Settings watcher error:`, error)
        this.watcher?.close()
        this.watcher = null
      })
    } catch (error) {
