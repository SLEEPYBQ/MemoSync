...
import { APP_NAME, CLI_COMMAND, getDataDirDisplay, LOG_PREFIX, PACKAGE_NAME, SELF_UPDATE_ENABLED } from "../shared/branding"
...
export function parseArgs(argv: string[]): ParsedArgs {
  // .env's PORT is the documented override for `bun run start`; --port wins.
  let port = Number(process.env.PORT) || PROD_SERVER_PORT
  let host = "127.0.0.1"
...
async function maybeSelfUpdate(_argv: string[], deps: CliRuntimeDeps) {
  ...
  if (process.env.MEMOSYNC_DISABLE_SELF_UPDATE === "1") {
    return null
  }
...
export function classifyInstallVersionFailure(output: string): UpdateInstallAttemptResult {
  ...
  return {
    ...
    userMessage: "MemoSync could not install the update. Try again later.",
  }
}
...
export function installPackageVersion(packageName: string, version: string) {
  if (!hasCommand("bun")) {
    return {
      ...
      userMessage: "MemoSync could not find Bun to install the update.",
