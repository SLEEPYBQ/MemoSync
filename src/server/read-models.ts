...
import { deployedProviders } from "./provider-catalog"

const SIDEBAR_RECENT_WINDOW_MS = 24 * 60 * 60 * 1_000
const SIDEBAR_FALLBACK_PREVIEW_LIMIT = 5

const MEMORY_RECORD_KINDS = new Set<TranscriptEntry["kind"]>([
  "memory_candidates",
  "memory_trace",
  "memory_preview",
  "memory_preview_decision",
  "memory_interrupt",
  "memory_interrupt_resolution",
  "memory_preview_relevance",
  "memory_preview_update",
  "memory_proposals",
  "memory_proposals_result",
  "memory_proposals_decision",
  "memory_checkup",
  "memory_checkup_result",
  "memory_checkup_decision",
  "memory_transfer",
  "memory_transfer_result",
  "memory_transfer_decision",
  "memory_preparation_reset",
])

/** Keep the Memory Record complete without rebroadcasting tool-heavy history. */
export function deriveMemoryRecordTranscript(entries: TranscriptEntry[] | null | undefined): TranscriptEntry[] {
  return (entries ?? []).filter((entry) => (
    MEMORY_RECORD_KINDS.has(entry.kind)
    || (entry.kind === "assistant_text" && /\[M-\d+\]/.test(entry.text))
  ))
}

export function deriveStatus(chat: ChatRecord, activeStatus?: ChatActivityStatus): ChatActivityStatus {
...
function canForkChat(
  chat: ChatRecord,
  activeStatuses: Map<string, ChatActivityStatus>,
...
export function deriveChatSnapshot(
  ...
  drainingChatIds: Set<string>,
  chatId: string,
  getMessages: (chatId: string) => Pick<ChatSnapshot, "messages" | "history" | "memoryRecordMessages">,
  ...
  const runtime: ChatRuntime = {
    ...
    streamingText: streamingTexts?.get(chat.id) ?? null,
  }
  ...
  return {
    ...
    memoryRecordMessages: transcript.memoryRecordMessages,
    history: transcript.history,
    availableProviders: deployedProviders(),
  }
}
