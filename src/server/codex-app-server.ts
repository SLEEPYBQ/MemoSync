...
interface PendingTurn {
  ...
  /** Execute a client-declared dynamic tool (e.g. memory tools) by name. */
  onDynamicToolCall?: (
    name: string,
...
/**
 * CODEX_MODEL overrides the per-chat model when set — this is how the Codex
 * engine runs against a custom model_provider (e.g. DeepSeek in Docker, with
...
/**
 * CODEX_PROTOCOL_COMPAT=0.9x adapts the app-server protocol for the PINNED
 * codex 0.92–0.94 window — the only versions that speak BOTH wire_api="chat"
 * (needed for DeepSeek, which has no /v1/responses) AND dynamicTools. Two
 * skews vs the vendored (newer) protocol:
 *   - collaborationMode.mode: "default" does not exist yet → "code"
 *   - item/tool/call response: { output, success } instead of { contentItems, success }
 */
function codexProtocolCompat09x(): boolean {
  return process.env.CODEX_PROTOCOL_COMPAT === "0.9x"
}

function dynamicToolCallResult(text: string, success: boolean): Record<string, unknown> {
  return codexProtocolCompat09x()
    ? { output: text, success }
    : ({ contentItems: text ? [{ type: "inputText", text }] : [], success } satisfies DynamicToolCallResponse)
}
...
export class CodexAppServerManager {
  ...
  async startTurn(args: StartCodexTurnArgs): Promise<HarnessTurn> {
    ...
    const pendingTurn: PendingTurn = {
      ...
      onDynamicToolCall: args.onDynamicToolCall,
    }
  ...
  private async handleServerRequest(context: SessionContext, request: ServerRequest) {
    ...
    if (request.method === "item/tool/call") {
      ...
      if (request.params.tool === "update_plan") {
        ...
        this.writeMessage(context, {
          id: request.id,
          result: dynamicToolCallResult("", true),
        })
        return
      }
      ...
      // Client-declared dynamic tools (e.g. MemoSync memory tools): dispatch to
      // the registered handler instead of rejecting. (Kanna's default is the
      // rejection below when no handler is wired.)
      if (pendingTurn.onDynamicToolCall) {
        const toolArgs = asRecord(request.params.arguments) ?? {}
        let text: string
        let isError = false
        try {
          const result = await pendingTurn.onDynamicToolCall(request.params.tool, toolArgs)
          text = result.text
        ...
        this.writeMessage(context, {
          id: request.id,
          result: dynamicToolCallResult(text, !isError),
        })
        return
      }
      ...
      this.writeMessage(context, {
        id: request.id,
        result: dynamicToolCallResult(errorMessage, false),
      })
      return
    }
